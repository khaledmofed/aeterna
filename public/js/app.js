  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      if (msgEl) {
        ...
        msgEl.className   = res.ok ? 'text-nexus-yellow text-sm mt-2' : 'text-red-400 text-sm mt-2';
        msgEl.style.display = 'block';
      }
      if (res.ok) form.reset();
    } catch {
      if (msgEl) { msgEl.textContent = 'An error occurred. Please try again.'; msgEl.style.display = 'block'; }
    } finally {
      btn.disabled = false;
    }
  });
...
function initHeroCanvas() {
  ...
  window.addEventListener('scroll', () => {
    if (wrapper) wrapper.style.transform = `translateY(${window.scrollY * 0.35}px)`;
  }, { passive: true });
