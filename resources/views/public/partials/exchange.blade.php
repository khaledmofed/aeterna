@php
$features = __('messages.exchange.features');
$steps    = __('messages.exchange.steps');
$featureIcons = [
...
<section id="exchange" class="py-24 px-6" style="background:#F5F4F0">
  <div class="max-w-7xl mx-auto">
    <div class="text-center mb-16" data-animate>
      <div class="section-label mb-4">{{ __('messages.exchange.badge') }}</div>
      <h2 class="text-4xl md:text-5xl mb-4" style="color:#1A1A1A;font-weight:900;letter-spacing:-0.03em">{!! nl2br(e(__('messages.exchange.title'))) !!}</h2>
      <p class="text-lg max-w-2xl mx-auto" style="color:#454745">{{ __('messages.exchange.subtitle') }}</p>
    </div>
          ...
          <div class="p-8 flex flex-col h-full relative z-10">
            <div class="flex justify-between items-start mb-6">
              <div class="w-12 h-12 rounded-lg flex items-center justify-center transition-all duration-300 group-hover:scale-110 group-hover:bg-[#9FE870] shrink-0"
                ...
                <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">{!! $featureIcons[$i] ?? '' !!}</svg>
              </div>
    ...
    <div class="rounded-2xl p-8" style="background:#FFFFFF;border:1px solid #D6D6D6" data-animate>
      <h3 class="text-xl font-bold mb-6" style="color:#1A1A1A">{{ __('messages.exchange.demo_title') }}</h3>
