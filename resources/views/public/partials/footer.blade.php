<footer style="background:#0D0D0D;border-top:1px solid #1A1A1A;padding-top:4rem;padding-bottom:2rem;padding-left:1.5rem;padding-right:1.5rem">
  <div class="max-w-7xl mx-auto">
    <div class="grid md:grid-cols-5 gap-10 mb-12">
      ...
      <div class="md:col-span-1">
        ...
        <div class="flex gap-3 mt-5">
          <a href="{{ \App\Models\SiteSetting::get('twitter_url','#') }}" target="_blank"
             class="w-9 h-9 rounded-full flex items-center justify-center transition-all duration-200 hover:border-[#EBFF00]/40 hover:text-[#EBFF00]"
             style="background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1);color:rgba(255,255,255,0.5)">
            <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
          </a>
          <a href="{{ \App\Models\SiteSetting::get('discord_url','#') }}" target="_blank"
             class="w-9 h-9 rounded-full flex items-center justify-center transition-all duration-200 hover:border-[#EBFF00]/40 hover:text-[#EBFF00]"
             style="background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1);color:rgba(255,255,255,0.5)">
          ...
          <a href="{{ \App\Models\SiteSetting::get('github_url','#') }}" target="_blank"
             class="w-9 h-9 rounded-full flex items-center justify-center transition-all duration-200 hover:border-[#EBFF00]/40 hover:text-[#EBFF00]"
             style="background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1);color:rgba(255,255,255,0.5)">
      ...
      <div>
        <h4 class="font-bold text-sm mb-4" style="color:#FFFFFF;letter-spacing:-0.01em">{{ $group }}</h4>
        <ul class="flex flex-col gap-2.5">
          @foreach($links as $link)
          <li>
            <a href="{{ $link->url }}" target="{{ $link->url === '#' ? '_self' : '_blank' }}"
               class="text-sm transition-colors duration-200 hover:text-[#EBFF00]"
               style="color:rgba(255,255,255,0.45)">{{ $link->label }}</a>
          </li>
          @endforeach
        </ul>
      </div>
