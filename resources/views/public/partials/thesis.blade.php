@php
$web2    = __('messages.thesis.web2_steps');
$aeterna = __('messages.thesis.aeterna_steps');
@endphp

<section id="thesis" class="py-24 px-6" style="background:#F5F4F0">
  <div class="max-w-6xl mx-auto">
    <div class="text-center mb-16" data-animate>
      <div class="section-label mb-4">{{ __('messages.thesis.badge') }}</div>
      <h2 class="text-4xl md:text-5xl font-bold mb-4" style="color:#1A1A1A;font-weight:900;letter-spacing:-0.03em">{{ __('messages.thesis.title') }}</h2>
      <p class="text-lg max-w-2xl mx-auto" style="color:#454745">{{ __('messages.thesis.subtitle') }}</p>
    </div>
    ...
    <div class="grid md:grid-cols-2 gap-8" data-animate>
      ...
      <div class="rounded-2xl p-8" style="background:#FFFFFF;border:1px solid rgba(239,68,68,0.25)">
        <h3 class="text-lg font-bold mb-6 flex items-center gap-2" style="color:rgb(239,68,68)">
          ...
          {{ __('messages.thesis.web2_title') }}
        </h3>
        @foreach($web2 as $i => $step)
        <div class="flex items-start gap-4 {{ $i < count($web2) - 1 ? 'mb-4' : '' }}">
          <div class="flex flex-col items-center">
            ...
            @if($i < count($web2) - 1)<div class="w-px h-8 my-1" style="background:rgba(239,68,68,0.15)"></div>@endif
          </div>
          <div class="pt-1.5">
      ...
      <div class="aeterna-flow-card rounded-2xl p-8" style="background:linear-gradient(135deg,rgba(159,232,112,0.16) 0%,#EEECEA 60%);border:1px solid rgb(159,232,112)">
        <h3 class="text-lg font-bold mb-6 flex items-center gap-2" style="color:#59d807">
          ...
          {{ __('messages.thesis.aeterna_title') }}
        </h3>
        @foreach($aeterna as $i => $step)
        <div class="flex items-start gap-4 {{ $i < count($aeterna) - 1 ? 'mb-4' : '' }}">
          <div class="flex flex-col items-center">
            ...
            @if($i < count($aeterna) - 1)<div class="w-px h-8 my-1" style="background:rgba(26,26,26,0.15)"></div>@endif
          </div>
          <div class="pt-1.5">
            <div class="font-medium text-sm" style="color:#1A1A1A">{{ $step[0] }}</div>
            <div class="text-xs mt-0.5" style="color:#454745">{{ $step[1] }}</div>
