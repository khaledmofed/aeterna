...
@section('content')
<div class="mb-4"><h1 class="h4 fw-bold text-white mb-0">{{ isset($item) ? 'Edit Nav Item' : 'New Nav Item' }}</h1></div>
...
<form method="POST" action="{{ isset($item) ? route('admin.navigation.update', $item) : route('admin.navigation.store') }}">
  ...
  <div class="admin-card p-4">
    ...
    <div class="d-flex gap-2">
      ...
      <a href="{{ route('admin.navigation.index') }}" class="btn btn-outline-secondary flex-fill">Cancel</a>
    </div>
  </div>
</form>
</div></div>
