...
@section('content')
<div class="mb-4"><h1 class="h4 fw-bold text-white mb-0">{{ isset($stage) ? 'Edit Stage' : 'New Stage' }}</h1></div>
...
<form method="POST" action="{{ isset($stage) ? route('admin.roadmap.update', $stage) : route('admin.roadmap.store') }}">
  ...
  <div class="admin-card p-4">
    <div class="row g-3 mb-3">
      <div class="col-4"><label class="form-label">Stage #</label>
    ...
    <div class="mb-3"><label class="form-label">Timeframe</label>
    ...
    <div class="mb-3"><label class="form-label">Milestones <small class="text-white-50">(one per line)</small></label>
    ...
    <div class="form-check form-switch mb-4">
      <input class="form-check-input" type="checkbox" name="is_active" value="1" id="is_active" {{ (old('is_active', $stage->is_active ?? true)) ? 'checked' : '' }}>
    ...
    <div class="d-flex gap-2">
      ...
      <a href="{{ route('admin.roadmap.index') }}" class="btn btn-outline-secondary flex-fill">Cancel</a>
    </div>
  </div>
</form>
</div></div>
