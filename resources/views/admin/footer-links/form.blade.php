...
@section('content')
<div class="mb-4"><h1 class="h4 fw-bold text-white mb-0">{{ isset($link) ? 'Edit Link' : 'New Link' }}</h1></div>
...
<form method="POST" action="{{ isset($link) ? route('admin.footer-links.update', $link) : route('admin.footer-links.store') }}">
  ...
  <div class="admin-card p-4">
    <div class="mb-3"><label class="form-label">Group</label>
      <input type="text" name="group_name" class="form-control" required placeholder="Developers" value="{{ old('group_name', $link->group_name ?? '') }}">
    </div>
    ...
    <div class="d-flex gap-2">
      ...
      <a href="{{ route('admin.footer-links.index') }}" class="btn btn-outline-secondary flex-fill">Cancel</a>
    </div>
  </div>
</form>
</div></div>
